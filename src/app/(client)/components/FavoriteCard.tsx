  ...
  const {favorites,handleFavorite,handleUnFavorite}=useFavorite()
    ...
    return(
      ...
      >
        ...
        {/* Cambiar logo */}
        <button className="absolute top-2 right-2" onClick={()=>isFavorite?handleUnFavorite({product,token}):handleFavorite({product,token})}>{isFavorite?<HeartFilled />:<Heart/>}</button>
      </article>
    )
}