import { ProductI } from "@/types";
import { HeartFilled } from "@/app/ui/icons/heartFilled";
import { useState } from "react";
import { Heart } from "lucide-react";
import { createFavorite, deleteFavorite } from "@/lib/fetch/favorites";


export function FavoriteCard({product,token}:{product:ProductI,token:string}){

    const [isFilled,setFilled]=useState(true)
    
    
    const handleFavorite=async({id,token}:{id:any,token:string})=>{
        createFavorite({token,productId:id})
        .then((res)=>setFilled(true))
        .catch(()=>setFilled(false))
        
      }
      
     const handleUnFavorite=async({id,token}:{id:any,token:string})=>{
         deleteFavorite({token,productId:id})
         .then((res)=>setFilled(false))
         .catch(()=>setFilled(true))
    }
    return(
        <article
        key={product.id}
        className="flex items-start gap-5 dark:bg-[#000000] bg-[#f7f0ee]  rounded-2xl relative h-32"
      >
        <img
          src={product.image}
          alt=""
          className="rounded-2xl w-36 h-32 object-cover"
        />
        <div className="flex flex-col justify-evenly gap-y-7 h-full pr-8">
          <h1 className="font-bold text-lg">{product.name}</h1>
          <div className="">
            <p className="overflow-x-hidden lg:w-full opacity-70 text-xs w-[95%] text-ellipsis">
              {product.description}
            </p>
            <p className="overflow-x-hidden lg:w-full opacity-70 text-xs w-[95%] text-ellipsis">
              {product.ingredients}
            </p>
          </div>
        </div>
        {/* Cambiar logo */}
        <button className="absolute top-2 right-2" onClick={()=>isFilled?handleUnFavorite({id:product.id,token}):handleFavorite({id:product.id,token})}>{isFilled?<HeartFilled />:<Heart/>}</button>
      </article>
    )
}