...
export function NotificaitonClient({
  dataNotifications,
  token,
}: {
  dataNotifications: dataNotificationI[];
  token: string;
}) {
  return (
      ...
      {dataNotifications.map((data) => {
        return (
          <Alert
            ...
            className={`${data.isSeen ? "" : "border-green-600"}`}
            onClick={() =>
              data.isSeen
                ? null
                : handleSeen({ token, notifyId: data.notification.id })
            }
          >
            <Sparkles className="stroke-yellow-500" />
