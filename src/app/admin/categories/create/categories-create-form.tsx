  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-8">
        ...
        <div>
          ...
          <Input id="name" name="name" type="text" required={true} onBlur={(e)=>setErrors(prev=>({...prev,[e.target.name]:"sdas"}))} />
        </div>
        <div>
          <Label htmlFor="file">Imagen</Label>
          <Input type="file" id="file" name="file" required={true} onChange={handleImage}/>
        </div>
        <div className="border-[1px] w-3/12 ">
          {image &&<img src={URL.createObjectURL(image)} alt="" className="w-40"/>}
        </div>
